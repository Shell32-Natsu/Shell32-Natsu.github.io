...
可以看到，主机B的`6666`端口被转发到了主机A的`54321`端口，而主机A的`54321`端口就是socks代理的监听端口。关于脚本中的ssh命令选项在此就不赘述了。不过有一点需要注意的是，使用`sshpass`搭配ssh端口转发会导致ssh client开启转发后立刻退出，因此使用`sh -c ... sleep .1`来避免这个问题。
...
接下来是最后一个问题，阿里云函数在运行Custom Container的时候，默认通过`9000`端口传入参数，因为我们的镜像也必须有一个HTTP服务器监听`9000`端口。好在用Go语言写起来很容易：
